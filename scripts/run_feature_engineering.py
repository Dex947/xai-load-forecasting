...
def main():
    ...
    try:
        ...
        feature_categories = {
            "temporal": len(
                [
                    ...
                    if any(x in c for x in ["hour", "day", "week", "month", "season"])
                ]
            ),
            "calendar": len(
                [
                    c
                    for c in features.columns
                    if any(x in c for x in ["holiday", "weekend", "business"])
                ]
            ),
            ...
            "weather": len(
                [
                    ...
                    if any(
                        x in c
                        for x in ["temp", "humidity", "wind", "precip", "hdd", "cdd"]
                    )
                ]
            ),
        ...
        # Data quality check
        logger.info("\nData quality check:")
        missing_pct = (features.isna().sum() / len(features) * 100).sort_values(
            ascending=False
        )
        ...
        logger.info(
            f"  Memory usage: {features.memory_usage(deep=True).sum() / 1024**2:.1f} MB"
        )
        logger.info("\nNext steps:")
