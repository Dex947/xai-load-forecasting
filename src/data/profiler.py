...
class DataProfiler:
    ...
    def _analyze_missing_data(self) -> Dict:
        ...
        for col in self.df.columns:
            ...
            missing_stats[col] = {
                ...
                "first_missing": str(self.df[self.df[col].isna()].index[0])
                if missing_count > 0
                else None,
                "last_missing": str(self.df[self.df[col].isna()].index[-1])
                if missing_count > 0
                else None,
            }
    ...
    def _temporal_coverage(self) -> Dict:
        ...
        expected_index = pd.date_range(
            start=self.df.index.min(), end=self.df.index.max(), freq="H"
        )
        ...
        return {
            ...
            "coverage_ratio": len(self.df) / len(expected_index)
    ...
    def _load_statistics(self) -> Dict:
        ...
        return {
            ...
            "cv": float(load_data.std() / load_data.mean())
            if load_data.mean() != 0
            else None,
        }
    ...
    def _analyze_autocorrelation(self, max_lags: int = 168) -> Dict:
        ...
        return {"autocorrelation": acf_values, "max_lag_analyzed": max_lags}
    ...
    def _detect_outliers(self, std_threshold: float = 5) -> Dict:
        ...
        iqr = q3 - q1
        outliers_iqr = load_data[
            (load_data < q1 - 1.5 * iqr) | (load_data > q3 + 1.5 * iqr)
        ]
    ...
    def plot_load_patterns(
        self, save_dir: Optional[str] = None, figsize: Tuple[int, int] = (15, 10)
    ) -> None:
        """
        ...
        weekend_hourly = (
            ...
            .groupby(self.df[self.df["is_weekend"]].index.hour)[self.load_column]
            .mean()
        )
        weekday_hourly = (
            self.df[~self.df["is_weekend"]]
    ...
    def plot_missing_data_heatmap(
        self, save_dir: Optional[str] = None, figsize: Tuple[int, int] = (12, 6)
    ) -> None:
        """
        ...
        sns.heatmap(
            daily_missing.T, cmap="YlOrRd", cbar_kws={"label": "Missing Count"}, ax=ax
        )
        ax.set_title(
            "Missing Data Heatmap (Daily Aggregation)", fontsize=14, fontweight="bold"
        )
        ax.set_xlabel("Date")
    ...
    def plot_load_weather_scatter(
        ...
        ax.scatter(plot_df[weather_feature], plot_df[self.load_column], alpha=0.3, s=10)
        ...
        ax.set_ylabel("Load")
        ax.set_title(
            f"Load vs {weather_feature.replace('_', ' ').title()} (Correlation: {corr:.3f})"
        )
        ax.legend()
