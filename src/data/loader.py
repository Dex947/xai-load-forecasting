...
def load_load_data(
    ...
    try:
        if suffix == ".csv":
            df = pd.read_csv(
                file_path, parse_dates=[date_column] if parse_dates else None
            )
        elif suffix == ".parquet":
        ...
        logger.info(
            f"Load statistics: mean={df[load_column].mean():.2f}, "
...
def merge_load_weather(
    ...
    if validate_alignment:
        ...
        if load_df.index.tz != weather_df.index.tz:
            logger.warning(
                f"Timezone mismatch: load={load_df.index.tz}, weather={weather_df.index.tz}"
            )
    ...
    logger.info(
        f"Merged data range: {merged_df.index.min()} to {merged_df.index.max()}"
    )
