...
class SolarDataFetcher:
    ...
    def fetch(
        self, start_date: str, end_date: str, variables: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
        ...
        df.index = (
            df.index.tz_localize(self.timezone) if df.index.tz is None else df.index
        )
        ...
        logger.info(
            f"Fetched {len(df)} hours of solar data with {len(df.columns)} variables"
        )
        return df
