...
class TemporalValidator:
    ...
    def validate_no_future_leakage(
        ...
        """
        logger.info(
            f"Validating no future leakage for prediction time: {prediction_time}"
        )
    ...
    def validate_train_test_split(
        self, train_df: pd.DataFrame, test_df: pd.DataFrame, gap_days: int = 1
    ) -> bool:
        """
    ...
    for col in numeric_cols:
        if df[col].notna().sum() > 0:
            ...
            if outlier_ratio > 0.01:  # More than 1% outliers
                logger.info(
                    f"Column '{col}' has {outlier_count} outliers ({outlier_ratio:.2%})"
                )
...
def check_temporal_consistency(
    df: pd.DataFrame, expected_freq: str = "H"
) -> Dict[str, any]:
    """
    ...
    consistency_metrics["gaps"] = [
        str(ts) for ts in missing_timestamps[:10]
    ]  # First 10 gaps
    ...
    logger.info(
        f"Temporal consistency check complete: {consistency_metrics['n_gaps']} gaps, "
