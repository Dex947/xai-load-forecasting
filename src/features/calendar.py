...
class CalendarFeatureEngineer:
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.calendar_config.get("holidays", True):
            ...
            # Add custom holidays
            features_df["is_holiday"] |= index_dates.isin(self.custom_holidays).astype(
                int
            )

            logger.debug(
                f"Created is_holiday feature ({features_df['is_holiday'].sum()} holidays)"
            )
        ...
        if self.calendar_config.get("holiday_proximity", True):
            ...
            features_df["days_to_holiday"] = self._calculate_days_to_holiday(
                df.index, days_before, days_after
            )
        ...
        if self.calendar_config.get("special_events", True):
            ...
            logger.debug(
                f"Created special event feature ({features_df['is_special_event'].sum()} events)"
            )
        ...
        if self.holidays_config.get("weekend_adjacent", {}).get("friday_before", True):
            features_df["is_friday_before_weekend_holiday"] = (
                self._is_friday_before_weekend_holiday(df.index).astype(int)
            )

        if self.holidays_config.get("weekend_adjacent", {}).get("monday_after", True):
            features_df["is_monday_after_weekend_holiday"] = (
                self._is_monday_after_weekend_holiday(df.index).astype(int)
            )
    ...
    def _calculate_days_to_holiday(
        self, index: pd.DatetimeIndex, days_before: int, days_after: int
    ) -> pd.Series:
        """
