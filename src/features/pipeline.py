...
class FeaturePipeline:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.temporal_engineer = TemporalFeatureEngineer(self.config["features"])
    ...
    def create_all_features(
        self, df: pd.DataFrame, target_column: str = "load", include_target: bool = True
    ) -> pd.DataFrame:
        ...
        lag_hours = self.config["features"].get(
        ...
        rolling_windows = self.config["features"].get(
        ...
        if weather_columns:
            ...
            weather_lag_hours = self.config["features"].get(
                "weather_lag_hours", [0, 1, 3, 6]
            )
        ...
        interaction_features = self.weather_engineer.create_interaction_features(
            all_features, interactions_config=interactions_config
        )
        ...
        logger.info(
            f"Feature engineering complete: {len(all_features.columns)} features created"
        )
    ...
    def _log_feature_summary(self, features_df: pd.DataFrame) -> None:
        ...
        for col in features_df.columns:
            if any(
                x in col for x in ["hour", "day", "week", "month", "quarter", "season"]
            ):
    ...
    def save_features(
        self, features_df: pd.DataFrame, file_path: str, format: str = "parquet"
    ) -> None:
        """
    ...
    def load_features(self, file_path: str, format: str = "parquet") -> pd.DataFrame:
        """
