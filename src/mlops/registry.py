...
class ModelRegistry:
        ...
        if tags:
            for key, value in tags.items():
                self.client.set_model_version_tag(
                    name=model_name, version=mv.version, key=key, value=value
                )
        ...
        if archive_existing:
            ...
            if current:
                self.client.transition_model_version_stage(
                    name=model_name, version=current.version, stage=self.STAGE_ARCHIVED
                )
                logger.info(f"Archived staging v{current.version}")

        self.client.transition_model_version_stage(
            name=model_name, version=str(version), stage=self.STAGE_STAGING
        )
    ...
    def promote_to_production(
        ...
        self.client.transition_model_version_stage(
            name=model_name, version=str(version), stage=self.STAGE_PRODUCTION
        )
    ...
    def rollback_production(self, model_name: str) -> bool:
        ...
        archived = [v for v in versions if v.current_stage == self.STAGE_ARCHIVED]
    ...
    def compare_versions(
        self, model_name: str, version_a: int, version_b: int, metrics: List[str]
    ) -> Dict[str, Dict]:
        """
