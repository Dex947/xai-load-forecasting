...
class ConformalForecaster:
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "ConformalForecaster":
        ...
        self.mapie_model = MapieTimeSeriesRegressor(
            self.base_model, method="enbpi", cv=cv, agg_function="mean", n_jobs=-1
        )
    ...
    def predict(
        self, X: pd.DataFrame, return_intervals: bool = True
        ...
        y_pred, y_intervals = self.mapie_model.predict(
            X.values, alpha=self.alpha, ensemble=True, optimize_beta=True
        )
    ...
    def predict_with_update(
        self, X: pd.DataFrame, y_true: Optional[pd.Series] = None, step_size: int = 1
        ...
        for i in range(step_size, n_samples, step_size):
            ...
            if y_true is not None:
                self.partial_fit(
                    X.iloc[i - step_size : i], y_true.iloc[i - step_size : i]
                )
    ...
    def evaluate(
        self, y_true: np.ndarray, y_pred: np.ndarray, intervals: np.ndarray
    ) -> Dict[str, float]:
        """
        ...
        """
        coverage = regression_coverage_score(y_true, intervals[:, 0], intervals[:, 1])

        mean_width = regression_mean_width_score(intervals[:, 0], intervals[:, 1])
