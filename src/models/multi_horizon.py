...
class MultiHorizonForecaster:
    ...
    def fit(
        self, X: pd.DataFrame, y: pd.Series, val_ratio: float = 0.2
    ...
    def evaluate(self, X: pd.DataFrame, y: pd.Series) -> Dict[int, Dict[str, float]]:
...
class RecursiveForecaster:
    ...
    def predict(
        self, X_last: pd.Series, horizon: int, feature_builder: callable
    ) -> np.ndarray:
        """
