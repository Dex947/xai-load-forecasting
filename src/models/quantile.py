...
class QuantileForecaster:
    ...
    def __init__(
        self, quantiles: List[float] = [0.1, 0.5, 0.9], config: Optional[Dict] = None
    ):
        """
        Args:
    ...
    def predict_interval(
        self, X: pd.DataFrame, lower_q: float = 0.1, upper_q: float = 0.9
        ...
        lower = (
            preds[lower_col].values if lower_col in preds else preds.iloc[:, 0].values
        )
        upper = (
            preds[upper_col].values if upper_col in preds else preds.iloc[:, -1].values
        )
        median = (
            preds[median_col].values
            if median_col in preds
            else preds.median(axis=1).values
        )

        return lower, median, upper

    def evaluate_coverage(
        self, X: pd.DataFrame, y: pd.Series, lower_q: float = 0.1, upper_q: float = 0.9
    ) -> Dict[str, float]:
