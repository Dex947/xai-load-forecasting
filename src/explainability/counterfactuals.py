...
class CounterfactualExplainer:
    ...
    def __init__(
        ...
        self.dice_model = dice_ml.Model(
            model=self._create_model_wrapper(), backend="sklearn"
        )
        ...
        try:
            ...
            return {
                ...
                "summary": self._generate_summary(
                    changes, current_pred, desired_outcome
                ),
            }
    ...
    def _analyze_changes(
        self, original: pd.DataFrame, counterfactuals: pd.DataFrame
    ) -> List[Dict]:
                ...
                if col in cf_row.index:
                    ...
                    if orig_val != cf_val:
                        ...
                        if isinstance(orig_val, (int, float)) and isinstance(
                            ...
                            change["pct_change"] = (
                                float((cf_val - orig_val) / orig_val * 100)
                                if orig_val != 0
                                else 0
                            )
    ...
    def _generate_summary(
        self, changes: List[Dict], current: float, desired: float
    ) -> str:
        ...
        if not feature_counts:
            return (
                f"To {direction} load by {delta:.2f} kW, no simple changes were found."
            )
    ...
    def what_if_analysis(
        self, base_instance: pd.DataFrame, feature_changes: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "pct_change": float((new_pred - base_pred) / base_pred * 100)
            if base_pred != 0
            else 0,
    ...
    def sensitivity_analysis(
        self, base_instance: pd.DataFrame, feature: str, values: List[float]
    ) -> pd.DataFrame:
        """
