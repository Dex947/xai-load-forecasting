...
class DriftExplainer:
    ...
    def __init__(
        ...
        } if total > 0 else baseline_importance
    ...
    def analyze_drift(
        ...
        } if total > 0 else current_importance
        
        # Compute changes for each feature
        changes = []
        for feature in set(self.baseline_normalized.keys()) | set(current_normalized.keys()):
    ...
    def explain_drift(self, drift_result: Dict[str, Any]) -> str:
        ...
        if severity == "none":
            return "No significant concept drift detected. Feature importance is stable."
        
        explanation = f"**Concept Drift Detected** (severity: {severity}, score: {score:.3f})\n\n"
        ...
        for c in changes[:2]:
            ...
            if "temperature" in feat.lower():
                explanation += f"  - Seasonal weather pattern change affecting `{feat}`\n"
            elif "lag" in feat.lower():
                explanation += f"  - Change in load autocorrelation structure (`{feat}`)\n"
            elif "hour" in feat.lower() or "day" in feat.lower():
                explanation += f"  - Shift in temporal consumption patterns (`{feat}`)\n"
    ...
    def update_baseline(self, new_importance: Dict[str, float]):
        ...
        } if total > 0 else new_importance
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "baseline_top_features": sorted(
                self.baseline_normalized.items(),
                key=lambda x: -x[1]
            )[:self.top_k_features],
        }
