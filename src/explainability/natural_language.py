...
class NaturalLanguageExplainer:
    ...
    FEATURE_CATEGORIES = {
        ...
        "weather": ["temperature", "humidity", "wind", "pressure", "cloud", "ghi", "dni"],
    ...
    def __init__(
        ...
        logger.info(f"NaturalLanguageExplainer initialized: {len(feature_names)} features")
    ...
    def explain_prediction(
        ...
        if negative_impact:
            ...
            for feat, impact in negative_impact[:3]:
                desc = self._describe_feature_impact(feat, abs(impact), feature_values, negative=True)
                lines.append(f"  • {desc}")
            lines.append("")
    ...
    def _describe_feature_impact(
        ...
        if value is not None:
            if isinstance(value, (int, float)):
                ...
                return f"{readable_name} ({value_str}) is {direction} load by {impact_str}"
            else:
    ...
    def generate_summary_report(
        ...
        for feat, importance in top_features:
            ...
            lines.append(f"- **{desc}**: avg impact {importance:.3f} {self.target_unit}")
        lines.append("")
    ...
    def generate_alert_message(
        ...
        time_str = timestamp.strftime("%H:%M on %b %d") if timestamp else "upcoming period"
